...
//Consulta02
SELECT tb_curso.nome, tb_turma.numero, tb_aluno.nome, tb_aluno.cpf
FROM tb_curso
...
//Consulta03
SELECT tb_avaliacao.data, tb_avaliacao.nota, tb_aluno.nome, tb_resultado.nota_obtida, ROUND(CAST((tb_resultado.nota_obtida * 100) / tb_avaliacao.nota AS numeric),2) AS porcentagem
FROM tb_avaliacao
...
FROM tb_aluno
INNER JOIN tb_matricula ON tb_matricula.aluno_id = tb_aluno.cpf
INNER JOIN tb_turma ON tb_turma.id = tb_matricula.turma_id
INNER JOIN tb_avaliacao ON tb_avaliacao.turma_id = tb_turma.id
INNER JOIN tb_resultado ON tb_resultado.avaliacao_id = tb_avaliacao.id AND tb_resultado.aluno_id = tb_aluno.cpf
